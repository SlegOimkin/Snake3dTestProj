...
import { SpaceSkySystem } from "./space-sky";
...
export class SceneBuilder {
  ...
  private skySystem: SpaceSkySystem;
  ...
  constructor(
    ...
    this.scene.background = new Color("#020611");
    this.scene.fog = new FogExp2("#0a1c33", 0.0029);

    this.camera = new PerspectiveCamera(62, 1, 0.1, 240);
    this.camera.position.set(0, 6, -12);

    this.root = new Group();
    this.skySystem = new SpaceSkySystem(this.currentQuality, this.camera);
    this.scene.add(this.skySystem.root);
  ...
  applyQuality(quality: QualityConfig): void {
    this.currentQuality = quality;
    this.skySystem.applyQuality(quality);
  ...
  updateFromSnapshot(snapshot: SessionSnapshot): void {
    ...
    this.skySystem.update(snapshot.elapsedSec, this.unwrappedHead);
  }
  ...
  dispose(): void {
    ...
    this.scene.remove(this.skySystem.root);
